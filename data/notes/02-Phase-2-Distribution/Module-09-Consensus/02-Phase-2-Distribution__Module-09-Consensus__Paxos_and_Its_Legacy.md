...
Before studying what each phase does, it helps to understand why two phases are necessary at all. The intuition matters more than the mechanics — a reader who understands the safety argument can reconstruct the protocol; a reader who memorized the steps cannot.

**The single-phase race condition.** Imagine the simplest possible consensus protocol: every proposer broadcasts its value and a proposal number directly to all acceptors as `Accept(n, v)`. Acceptors accept the first proposal with the highest number they see from a majority. The problem: proposer A with value `X` and proposer B with value `Y` can split the vote. A reaches acceptors 1 and 2; B reaches acceptors 2 and 3. Acceptor 2 accepts B's proposal because it arrived first. Now A has promises from acceptors 1 and 2 — but acceptor 2 already accepted B. If we allow both to "succeed," two different values are chosen simultaneously. Consensus is violated.

**What Phase 1 (Prepare) actually buys.** Phase 1 is a *reconnaissance mission*, not a vote. By broadcasting `Prepare(n)`, the proposer asks every acceptor: "Have you already accepted anything?" If any acceptor has previously accepted a value, it returns that value in its response. The proposer is then *obligated* to adopt that value in Phase 2 rather than propose its own. This is the critical safety invariant: at most one value can ever be chosen, because any proposer that starts Phase 2 after a value has already been chosen by a majority will discover that value during Phase 1 and continue with it. Phase 1 also forces acceptors to reject any future `Accept` message with a lower number, preventing older proposals from "sneaking in" after Phase 1 completes.

**A concrete scenario.** Proposer A sends `Prepare(100)`. Acceptors 1, 2, and 3 promise to ignore proposals numbered below 100. A sends `Accept(100, X)`. Acceptors 1 and 2 accept; acceptor 3 is partitioned. Value `X` is chosen by the majority. Now proposer B wakes up and sends `Prepare(101)`. Acceptors 1 and 2 respond with "I already accepted `(100, X)`." Under the protocol rules, B *must* use `v = X` in its `Accept(101, X)` message. Even though B is a new proposer with a higher ballot number, the previously-chosen value survives intact. Without Phase 1's discovery step, B would send `Accept(101, Y)` and choose a conflicting value.

**The livelock flip side.** Phase 1's protection creates a livelock risk: two proposers alternately preempt each other's Phase 1 by escalating ballot numbers, and no `Accept` ever reaches a quorum. This is the "dueling proposers" failure described below. The fix — a stable designated leader (Multi-Paxos) that skips Phase 1 entirely once established — is why every production Paxos system is really Multi-Paxos. The two-phase structure is necessary for safety; the stable leader is necessary for liveness.
